...
export function CableRoutingGrid({
  ...
  const [editDragMode, setEditDragMode] = useState<"blank" | "activate" | null>(null);
  const lastEditIdx = useRef<number | null>(null);
  ...
  // ── Reset layout editing when leaving layout mode ─────────────────────────
  useEffect(() => {
    if (routingMode !== "layout") setIsEditingLayout(false);
  }, [routingMode]);
  ...
  function clientToPanel(e: { clientX: number; clientY: number }, allowBlanks = false): number | null {
    ...
    if (!allowBlanks && blankCells.includes(idx)) return null;
    return idx;
  }
  ...
  function handleMouseDown(e: React.MouseEvent<SVGSVGElement>) {
    if (routingMode === "layout" && isEditingLayout) {
      const idx = clientToPanel(e, true);
      if (idx === null) return;
      const isBlank = blankCells.includes(idx);
      const mode: "blank" | "activate" = isBlank ? "activate" : "blank";
      setEditDragMode(mode);
      isEditDragging.current = true;
      lastEditIdx.current = idx;
      if (mode === "blank") onBlankCellsChange([...blankCells, idx]);
      else onBlankCellsChange(blankCells.filter(i => i !== idx));
      return;
    }
    if (routingMode !== "data" && routingMode !== "power") return;
  ...
  function handleMouseMove(e: React.MouseEvent<SVGSVGElement>) {
    if (routingMode === "layout" && isEditingLayout && isEditDragging.current) {
      const idx = clientToPanel(e, true);
      if (idx === null || idx === lastEditIdx.current) return;
      lastEditIdx.current = idx;
      const isBlank = blankCells.includes(idx);
      if (editDragMode === "blank" && !isBlank) onBlankCellsChange([...blankCells, idx]);
      else if (editDragMode === "activate" && isBlank) onBlankCellsChange(blankCells.filter(i => i !== idx));
      return;
    }
    ...
    <div className="space-y-3">
      ...
      {routingMode === "layout" && (
        <div className="flex items-center gap-2">
          ...
          >
            {isEditingLayout ? "✓ Done editing" : "✎ Edit panels"}
          </button>
          {isEditingLayout && (
          ...
          {!isEditingLayout && blankCells.length > 0 && (
            <span className="text-xs text-gray-400 dark:text-gray-500">
              {blankCells.length} blank panel{blankCells.length !== 1 ? "s" : ""}
              <button
      ...
      <div className="relative">
        <svg
          ...
          style={{
            maxHeight: 520,
            cursor: (routingMode === "layout" && isEditingLayout)
              ? "crosshair"
              : (routingMode === "data" || routingMode === "power")
